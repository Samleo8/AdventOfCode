...
for(i=0;i<a.length;i++){
	...
	for(k in checkSum){
		checkSum[k].foundAlr = false;
	}

	for(j in freq){
		if(!freq.hasOwnProperty(j)) continue;

		if(checkSum[2].foundAlr && checkSum[3].foundAlr){ //both found already
			break;
		}

		for(k in checkSum){
			if(!checkSum.hasOwnProperty(k)) continue;

			if(!checkSum[k].foundAlr && freq[j] == parseInt(k)){
				console.log(i,j,k);
				checkSum[k].foundAlr = true;
				checkSum[k].total++;
			}
		}
	}
...
console.log("Ans:",checkSum[2].total*checkSum[3].total);
