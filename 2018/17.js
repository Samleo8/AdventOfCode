...
function parseAndPrintInput(print){
	var i,j,k;
	var _x,_y;
	var x1,x2,y1,y2;
...
Function for downward and side flow
Reason for different functions is because of different implementations (top-down and bottom-up). Side flow has to be done first before and downFlow can begin

	@param
		(x,y): exact coordinates according to input
			NOTE: the coordinates for the map (_X,_Y) are not the same as the coordinates (x,y).
			(_X,_Y) = (x-bounds.x.min,y)

	@return: how much water has filled
*/

var stop = false;

function downFlow(x,y){
	//OUT OF BOUNDS
	if(x<bounds.x.min || x>bounds.x.max || y>bounds.y.max){ //out of bounds
		return 0;
	}
	if(y<bounds.y.min && x!=springCoord.x){ //out of bounds, but not drop of water from spring
		return 0;
	}
...
/*=======FUNCTION: paint(x,y)============
Paints box at coordinate (x,y), as per input, for visualisation
	@param
		x,y: coordinates as per input
...
/*=======FUNCTION: arr.searchClosest(item)============
	@pre-condition: arr must be sorted!
	@param
		arr: sorted array to find

		item: item to be found in SORTED array
		ind1, ind2: indexes to search between
			(if `null`, ind1 = 0 and ind2 = arr.length-1)


	@return: index of item before and after
*/
/*
Array.prototype.searchClosest = function(item, ind1, ind2){
	if(ind1 == null || typeof ind1 == "undefined" || ind1<0) ind1 = 0;
	if(ind2 == null || typeof ind2 == "undefined" || ind2>=this.length) ind2 = this.length-1;

	var middleInd = parseInt((ind1+ind2)/2);
